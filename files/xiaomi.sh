...
fi_flash_images() {
	...
	mtd erase "$FI_ROOTFS_PART" || {
		fierr "Failed to erase partition '$part_name'"
		return 1
	}
	...
	filog "Kernel image flashed to '$FI_KERNEL_PART'"
	...
	filog "Rootfs image flashed to '$FI_ROOTFS_PART'!"
	...
	if [ "$part_skip" -gt 0 ]; then
		...
		filog "Rootfs image flashed to '$FI_ROOTFS_PART':$FI_ROOTFS_PARTSIZE"
	fi
